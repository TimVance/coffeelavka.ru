...
class Theme_admin extends Diafan
{
	...
	public function show_h1()
	{
		...
		if($this->diafan->_users->roles('init', 'site'))
		{
			...
			{
				$name = DB::query_result("SELECT [name] FROM {site} WHERE id=%d", $this->diafan->_route->site);
				if($this->diafan->_users->roles('init', 'site'))
				{
					echo '<span class="heading__in">
						'.$this->diafan->_('Подключен к странице').': <a href="'.BASE_PATH_HREF.'site/edit'.$this->diafan->_route->site.'/">'.$name.'</a>
					</span>';
				}
			}
			elseif ($row = DB::query_fetch_array("SELECT id, [name] FROM {site} WHERE module_name='%h' AND trash='0' LIMIT 1", $this->diafan->_admin->module))
			{
				if($this->diafan->_users->roles('init', 'site'))
				{
					echo '<span class="heading__in">
	...
	public function show_include($attributes)
	{
		...
		include_once(ABSOLUTE_PATH.'themes/blocks/'.$attributes["file"].'.php');
	}
