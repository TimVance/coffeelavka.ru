...
class Save_admin extends Diafan
{
	...
	public function save()
	{
		...
		if ($this->diafan->config("config"))
		{
			...
			for ($q = 0; $q < count($this->value); $q++)
			{
				...
				if (! $this->diafan->_route->site && $this->value[$q] || $this->diafan->_route->site && DB::query_result("SELECT value FROM {config} WHERE module_name='%s' AND site_id=0 AND name='%h'".($this->diafan->variable_multilang($name) ? " AND lang_id='"._LANG."'" : '' )." LIMIT 1", $this->diafan->_admin->module, $name) != $this->value[$q])
				{
	...
	public function save_new()
	{
		...
		{
			$def['parent_id'] = "'".$this->diafan->_route->parent."'";
		}
