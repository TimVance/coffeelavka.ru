...
class Show_admin extends Diafan
{
	...
	public function get_heading()
	{
		...
		foreach($this->diafan->variables_list as $row)
		{
			...
			echo '<div class="item__th'.(! empty($row["class_th"]) ? ' '.$row["class_th"] : '').'">'.(! empty($row["name"]) ? $row["name"] : '').'</div>';
		}
		echo '</li>';
	}
		...
		if ($this->diafan->config("element_multiple"))
		{
			if(! isset($this->cache["prepare"]["parent_cats"]))
			{
				$this->cache["prepare"]["parent_cats"] = DB::query_fetch_key_array(
					"SELECT s.[name], c.element_id FROM {".$this->diafan->_admin->module."_category_rel} as c"
