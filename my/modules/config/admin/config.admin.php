	...
	public $variables = array (
		'base' => array (
			...
			'userfiles' => array(
				...
				'help' => 'Имя папки, где будут храниться все загружаемые файлы для контента сайта.  По умолчанию все пользовательские файлы хранятся в папке *userfiles*. (Веб-мастеру и программисту. Не меняйте этот параметр, если не уверены в результате!)',
			),
			'admin_folder' => array(
				'type' => 'text',
			...
			'mobile_version' => array(
				'type' => 'checkbox',
				'name' => 'Разрешить мобильную версию сайта',
				'help' => 'Если отмечено, то CMS будет автоматически определять устройство, с которого зашли на сайт и если это мобильное устройство, то автоматически будет загружаться упрощенный шаблон дизайна themes/m/site.php.',
			),
			'no_x_frame' => array(
				'type' => 'checkbox',
				'name' => 'Разрешить загружать сайт во frame',
				'help' => 'Если отмечено, то сайт можно будет вставлять во frame. Не рекомендуется, т.к. повышается опасность clickjacking-атак на сайт.',
			),
			'hr3' => 'hr',
			'route_method' => array(
				'type' => 'select',
	...
	public function get_values()
	{
		if(defined('IS_DEMO') && IS_DEMO)
		{
			$values = array(
				...
				'userfiles' => ! empty($_SESSION["CONFIG_USERFILES"]) ? $_SESSION["CONFIG_USERFILES"] : 'userfiles',
