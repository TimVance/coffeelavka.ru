...
class Ab_admin extends Frame_admin
{
	...
	public $variables = array (
		...
		'other_rows' => array (
			...
			'prior' => array(
				...
				'help' => 'Если отмечена, новость выведется в начале списка, независимо от сортировки по дате.  Если важных объявлений несколько, между собой они будут сортироваться по дате.',
			),
			'map_no_show' => array(
				'type' => 'checkbox',
	...
	public function save_variable_param()
	{
		...
		foreach ($rows as $row)
		{
			...
			$id_param = DB::query_result("SELECT id FROM {ab_param_element} WHERE param_id=%d AND element_id=%d LIMIT 1", $row["id"], $this->diafan->id);
