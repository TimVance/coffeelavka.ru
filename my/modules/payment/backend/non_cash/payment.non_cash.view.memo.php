...
<table width="720" bordercolor="#000000" style="border:#000000 1px solid;" cellpadding="0" cellspacing="0">
	<?php
		...
		echo '<td><b>'.(!empty($values['summ']) ? $values['summ'] : '').'</b></td></tr>';
	?>
<table>
...
<?php echo $this->diafan->_('email', false);?>: <?php echo (!empty($user["email"]) ? $user["email"] : '');?><br>
<?php echo $this->diafan->_('Способ доставки', false);?>: <?php echo $values["delivery"]["name"];?><br>

<?php
//способ оплаты для товарной накладной
$payment_order_id = $values["order_id"];
$payment_id = DB::query_result("SELECT payment_id FROM {payment_history} WHERE element_id=$payment_order_id");
$payment_name = DB::query_result("SELECT name1 FROM {payment} WHERE id=$payment_id");
?>

<?php echo $this->diafan->_('Способ оплаты', false);?>: <?php echo $payment_name;?><br>
