...
//order_id=%d WHERE a.trash='0'", $row["id"]
///////////////////////////
//element_id
//способ оплаты для товарной накладной
$pays = DB::query_fetch_all("SELECT element_id, payment_id FROM {payment_history} WHERE element_id=$values[order_id]");
 // выполняем SQL-запрос к базе данных
//$result77 = DB::query_result("SELECT * FROM {payment_history}");
// получаем результат
//$id88 = DB::result($result77);
...
$rows = DB::query_fetch_all("SELECT name1 FROM {payment} WHERE id=6");
foreach ($rows as $row)
{
