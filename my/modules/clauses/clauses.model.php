...
class Clauses_model extends Model
{
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act] FROM {clauses_category}"
	...
	private function id_query($time)
	{
		...
		." [keywords], [descr], [canonical], site_id, [title_meta], access, theme, view, sort, [act]".$fields." FROM {clauses}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
	...
	public function show_block($count, $site_ids, $cat_ids, $sort, $images, $images_variation, $tag)
	{
		...
		if ($sort == "rand" || ! $result = $this->diafan->_cache->get($cache_meta, "clauses"))
		{
			...
			if($sort == 'keywords')
			{
				...
				$inner .= " INNER JOIN {search_results} AS r ON r.element_id=e.id AND r.table_name='clauses'";
