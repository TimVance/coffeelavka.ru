...
$i = 0;
// начало уровня меню
echo '<ul class="dropdown-menu  fullwidthmenu">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	)
	{
		...
		{
			echo '<a class="bold_link" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"])
	{
		...
		echo $this->get('show_level_cat_3', 'menu', $menu_data);
	}
	if ($row["active"])
	{
