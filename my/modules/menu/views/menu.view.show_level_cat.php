...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	)
	{
		...
		elseif ($row["children"])
		{
			echo '<a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"]
			.(!empty($row["active"]) || !empty($row["active_child"]) ? 'class="active"' : '')
			.'>';
		}
		else
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"])
	{
		...
		echo $this->get('show_level_cat_2', 'menu', $menu_data);
	}
