...
// начало уровня меню
echo '<ul class="dropdown-menu_mod">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="bold_link3" href="'.$row["othurl"].'"'.$row["attributes"].'>';
		}
		else
		{
			echo '<a class="bold_link3" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
