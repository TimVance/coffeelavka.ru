...
class Users_admin_role extends Frame_admin
{
	...
	private function table_tr_admin_roles($row, $variable_roles, $parent = true)
	{
		...
		foreach ($variable_roles as $v)
		{
			echo '<td><input type="checkbox" id="input_check_all_role_'.$v.'_'.$row["rewrite"].'" name="'.$v.'[]" value="'.$row["rewrite"].'" class="checkbox checkbox_'.$v.'"';
			if ($row["role"][$v])
			{
				echo " checked";
			}
	...
	private function table_site_roles()
	{
		...
		foreach ($this->user_variable_roles as $module => $roles)
		{
			...
			{
				echo '<input type="checkbox" name="'.$module.'[]" id="input_'.$module.'_'.$name.'" value="'.$name.'"'.(!empty($values[$module]) && in_array($name, $values[$module]) ? 'checked' : '').'> <label for="input_'.$module.'_'.$name.'">'.$this->diafan->_($title).'</label><br/>';
			}
	...
	{
		DB::query("DELETE FROM {users_role_perm} WHERE role_id=%d AND type='%h'", $this->diafan->id, $_POST['type']);
		if ($_POST['type'] == 'admin')
		{
			...
			if($all_roles)
			{
				DB::query("INSERT INTO {users_role_perm} (rewrite, perm, role_id, type) VALUES ('all', 'all', %d, '%s')", $this->diafan->id, $_POST['type']);
				return;
			}
			...
			foreach ($admin_pages as $admin_page)
			{
				...
				if (count($current_perm) > 0)
				{
					...
					DB::query("INSERT INTO {users_role_perm} (rewrite, perm, role_id, type) VALUES ('%s', '%s', %d, '%s')", $admin_page, $perm, $this->diafan->id, $_POST['type']);
				}
			}
		}
		...
		if ($_POST['type'] == 'site')
		{
			...
			foreach ($this->user_variable_roles as $module => $roles)
			{
				if (!empty($_POST[$module]))
				{
					...
					DB::query("INSERT INTO {users_role_perm} (rewrite,perm,role_id,type) VALUES ('%s','%s',%d,'%s')", $module, $perm, $this->diafan->id, $_POST['type']);
				}
			}
		}
	}
