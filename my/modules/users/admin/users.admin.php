...
class Users_admin extends Frame_admin
{
	...
	public $variables = array (
		'main' => array (
			...
			'useradmin' => array(
				'type' => 'checkbox',
				'name' => 'Подключить панель быстрого редактирования',
				'help' => 'Если отмечено, в пользовательской части сайта в шапке будет выводиться панель, с помошью которой можно редактировать контент прямо на сайте.',
			),
			'htmleditor' => array(
				'type' => 'checkbox',
	...
	public function edit_variable_password()
	{
		...
		<div class="unit" id="password">
			...
			<input type="password" name="password" value="" placeholder="Пароль сохранен. Введите новый для изменения">
		</div>';
	}
	...
	public function save_variable_phone()
	{
		...
		$this->diafan->set_value($_POST["phone"]);
	}
