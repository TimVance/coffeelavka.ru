...
class News_model extends Model
{
	...
	private function list_category_query()
	{
		...
		"SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act] FROM {news_category}"
	...
	private function id_query($time)
	{
		...
		$row = DB::query_fetch_array(
		"SELECT id, [name], [anons], [anons_plus], [text], timeedit, created, cat_id, [keywords], [descr], [canonical], site_id, [title_meta], access, theme, view, prior, [act]".$fields." FROM {news}"
	...
	public function show_block($count, $site_ids, $cat_ids, $sort, $images, $images_variation, $tag)
	{
		...
		if (! $result = $this->diafan->_cache->get($cache_meta, "news"))
		{
			...
			if($sort == 'keywords')
			{
				...
				$inner .= " INNER JOIN {search_results} AS r ON r.element_id=e.id AND r.table_name='news'";
				$inner .= " INNER JOIN {search_index} AS i ON r.id=i.result_id AND i.keyword_id IN ('".implode("', '", $keys)."') AND i.rating=0";
			}
