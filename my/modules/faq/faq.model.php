...
class Faq_model extends Model
{
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act] FROM {faq_category}"
	...
	private function id_query($time)
	{
		...
		." [descr], [canonical], [anons], [text], site_id, [title_meta], access, theme, view, [act]".$fields." FROM {faq}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
