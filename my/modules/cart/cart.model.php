...
class Cart_model extends Model
{
	...
	public function form()
	{
		...
		if($this->result["payments"])
		{
			foreach($this->result["payments"] as $i => $row)
			{
				...
				{
					if($this->diafan->_balance->get() < $this->result["summ"])
					{
	...
	public function form_table()
	{
		...
		foreach ($cart as $good_id => $array)
		{
			...
			foreach ($array as $param => &$ar)
			{
				...
				foreach ($ar as $additional_cost => &$c)
				{
					...
					if($additional_cost)
					{
						$additional_cost_rels = DB::query_fetch_all("SELECT a.id, a.[name], a.percent, a.price, r.element_id, r.summ FROM {shop_additional_cost} AS a INNER JOIN {shop_additional_cost_rel} AS r ON r.additional_cost_id=a.id WHERE r.element_id=%d AND a.id IN (%s) AND a.trash='0'", $good_id, $additional_cost);
		...
		// способы доставки
		$this->result["delivery"] = DB::query_fetch_all("SELECT id, [name], [text] FROM {shop_delivery} WHERE [act]='1' AND trash='0' ORDER BY sort ASC");
