...
class Cart_action extends Action 
{
	...
	public function recalc()
	{
		...
		$this->result["data"] = array(
			...
			".cart_summary" => $this->diafan->_tpl->get('total', 'cart', $form),
		);
		...
		foreach ($cart as $good_id => $array)
		{
			foreach ($array as $param => $ar)
			{
				foreach ($ar as $additional_cost => $c)
				{
					...
					if($additional_cost)
					{
						$additional_costs = DB::query_fetch_all("SELECT a.id, a.percent, a.price, r.summ FROM {shop_additional_cost} AS a INNER JOIN {shop_additional_cost_rel} AS r ON r.additional_cost_id=a.id WHERE r.element_id=%d AND a.id IN (%s) AND a.trash='0'", $good_id, $additional_cost);
					}
					else
					{
	...
	public function one_click()
	{
		if(! empty($_POST["good_id"]))
		{
			...
			if(! empty($_POST["additional_cost"]))
			{
				...
				if($a_cs)
				{
					$additional_cost = DB::query_fetch_value("SELECT additional_cost_id FROM {shop_additional_cost_rel} WHERE element_id=%d AND trash='0' AND additional_cost_id IN (%s)", $good_id, implode(',', $a_cs), "additional_cost_id");
	...
	private function send_sms()
	{
		...
		if (!empty($_POST["p3"]))
		{
			$message1 = 'Ваш заказ получен.';
			$to1 = $_POST["p3"];
			Sms::send($message1, $to1);
		}
	}
}