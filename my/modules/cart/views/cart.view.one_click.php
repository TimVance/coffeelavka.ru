	...
	foreach ($result["rows_param"] as $row)
	{
		...
		switch ($row["type"])
		{
			...
			case 'text':
				...
				<input type="text" class="form-control" name="p'.$row["id"].'" value="'.$value.'">';
				break;
			...
			case 'datetime':
				$timecalendar  = true;
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
					<input type="mail" name="p'.$row["id"].'" value="'.$value.'" class="timecalendar" showTime="'
...
echo '
<input type="button" value="'.$this->diafan->_('Заказать', false).'" class="btn btn-warning ">';
