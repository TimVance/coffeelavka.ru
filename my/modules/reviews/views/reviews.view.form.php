...
if (! empty($result["params"]))
{
	foreach ($result["params"] as $row)
	{
		...
		switch ($row["type"])
		{
			...
			case "email":
				echo '<div class="infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input type="mail" name="p'.$row["id"].'" value="'.$row["value"].'">';
				break;
			...
			case 'radio':
				...
				{
					echo '<input name="p'.$row["id"].'" type="radio" value="'.$select["id"].'" id="reviews_form_p'.$row["id"].'"'.($row["value"] == $select["id"] ? ' checked' : '').'> <label for="reviews_form_p'.$row["id"].'">'.$select["name"].'</label>';
				}
				break;
