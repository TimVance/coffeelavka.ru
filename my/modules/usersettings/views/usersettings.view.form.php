...
echo '<div class="row"><div class="col-md-6"><form action="'.$result["action"].'" method="POST" class="usersettings_form ajax" enctype="multipart/form-data">
...
<label>'.$this->diafan->_('ФИО или название компании').':</label>
<input type="text" class="form-control" name="fio" value="'.$result["fio"].'">
<div class="errors error_fio"'.($result["error_fio"] ? '>'.$result["error_fio"] : ' style="display:none">').'</div>
</div>

<div class="form-group">
<label>'.$this->diafan->_('E-mail').':</label>
<input type="email" class="form-control" name="mail" value="'.$result["mail"].'">
<div class="errors error_mail"'.($result["error_mail"] ? '>'.$result["error_mail"] : ' style="display:none">').'</div>
</div>

<div class="form-group">
<label>'.$this->diafan->_('Телефон').':</label>
<input type="tel" class="form-control" name="phone" value="'.$result["phone"].'">
<div class="errors error_phone"'.($result["error_phone"] ? '>'.$result["error_phone"] : ' style="display:none">').'</div></div>';

if($result["use_name"])
{
    echo '<div class="form-group">
    <label>'.$this->diafan->_('Логин').':</label>
	<input type="text" class="form-control" name="name" value="'.$result["name"].'">
	<div class="errors error_name"'.($result["error_name"] ? '>'.$result["error_name"] : ' style="display:none">').'</div></div>';
}
echo '<div class="form-group">
	<label>'.$this->diafan->_('Пароль').':</label>
	<input class="form-control" type="password" name="password">
	<div class="errors error_password"'.($result["error_password"] ? '>'.$result["error_password"] : ' style="display:none">').'</div></div>

	<div class="form-group">
	<label>'.$this->diafan->_('Повторите пароль').':</label>
...
<input type="submit" class="btn btn-warning btn-sm" value="'.$this->diafan->_('Сохранить', false).'">
...
<div class="errors usersettings_message"></div></div></div>';