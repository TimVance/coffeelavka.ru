...
class Usersettings_model extends Model
{
	...
	public function form()
	{
		...
		$matches = null;
		$value = preg_match('/(\\d{1})\\d{0}(\\d{3})\\d{0}(\\d{3})\\d{0}(\\d{2})\\d{0}(\\d{2})/', $this->diafan->_users->phone, $matches);			
		$this->result["phone"] = (!empty($matches) ? '+'.$matches[1].'-'.$matches[2].'-'.$matches[3].'-'.$matches[4].'-'.$matches[5] : '');
