...
class Dashboard_admin extends Frame_admin
{
	...
	public function show_h1()
	{
		if (! empty($_GET["help"])) {
	        ...
	        echo '<p>'.$this->diafan->_('Это модуль событий. Здесь выводятся уведомления об активности посетителей сайта. Вы можете изменить стартовую страницу в %s.', '<a href="'.BASE_PATH_HREF.'users/edit'.$this->diafan->_users->id.'/">своих настройках</a>').'</p>';
	        echo '<p>'.$this->diafan->_('Слева список модулей. Основной модуль - %s, служит для управления структурой. В %s можно править некоторые участки сайта, например, телефоны или счетчики.', '<a href="'.BASE_PATH_HREF.'site/">Страницы сайта</a>', '<a href="'.BASE_PATH_HREF.'site/blocks/">Блоки на сайте</a>').'</p>';
	        echo '<p>'.$this->diafan->_('С помощью модуля %s организована навигация по сайту.', '<a href="'.BASE_PATH_HREF.'menu/">Меню на сайте</a>').'</p>';
	        echo '<p>'.$this->diafan->_('Все остальные модули прикрепляются к Страницам сайта и/или являются их частью.').'</p>';
	        echo '<p><strong>'.$this->diafan->_('Смотрите краткое руководство %s', '<a href="http://www.diafan.ru/dokument/full-manual/introduction/pervoe_znakomstvo_s_panelyu_upravleniya_saytom/" target="_blank">Как управлять сайтом</a>').'</strong></p>';
	        echo '<p>'.$this->diafan->_('Возникающие вопросы задавайте в %s', '<a href="https://user.diafan.ru/support/" target="_blank">Службу поддержки</a>').'</p>';
	        echo '</div>';
	    }
