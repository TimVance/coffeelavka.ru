...
class Files_model extends Model
{
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act] FROM {files_category}"
	...
	private function id_query($time)
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus], [text], timeedit, cat_id, [keywords], [descr], [canonical], sort, [title_meta], access, theme, view, [act]".$fields." FROM {files}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
