...
class Shop_yandex extends Diafan
{
	...
	private function get_offers()
	{
		...
		foreach ($this->sites as $site_id)
		{
			$rows = DB::query_fetch_all("SELECT * FROM {shop} WHERE [act]='1' AND trash='0' AND site_id=%d"
			...
			foreach ($rows as $row)
			{
				...
				$img = DB::query_fetch_array("SELECT id, name, folder_num FROM {images} WHERE module_name='shop' AND"
								." element_id='%d' AND element_type='element' AND trash='0' ORDER BY sort ASC LIMIT 1", $row["id"]);
	
				$picture = $img ? BASE_PATH.USERFILES.'/shop/'.$GLOBALS['shop_images_variation_medium'].'/'.($img["folder_num"] ? $img["folder_num"].'/' : '').$img["name"] : '';
