...
class Shop_admin extends Frame_admin
{
	...
	private function params()
	{
		...
		$this->cache["params"] = DB::query_fetch_all("SELECT DISTINCT(p.id), p.[name], p.type, p.required, p.[measure_unit], p.[text], p.config FROM {shop_param} AS p INNER JOIN {shop_param_category_rel} AS r ON r.element_id=p.id AND r.cat_id IN (%s) WHERE p.trash='0' ORDER BY p.sort ASC", implode(',', $cat_ids));
