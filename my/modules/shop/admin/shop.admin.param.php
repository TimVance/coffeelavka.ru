...
class Shop_admin_param extends Frame_admin
{
	...
	public $variables = array (
		'main' => array (
			...
			'mainfilter' => array(
                'type' => 'checkbox',
                'name' => 'Показывать в фильтре на главной странице',
                'help' => 'Если отмечено, данная характеристика будет отображаться в фильтре на главной странице.',
                'default' => false,
            ),
	...
	public function save_variable_required()
	{
		...
		elseif(empty($_POST["required"]) && $this->diafan->values("required") || $_POST["type"] != 'multiple')
		{
			DB::query("DELETE FROM {shop_price_param} WHERE param_id=%d", $this->diafan->id);
		}
	}
