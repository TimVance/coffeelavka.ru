...
class Shop_admin_order extends Frame_admin
{
	...
	public function edit_variable_number()
	{
		...
		{
			echo '<b>'.$this->diafan->variable_name().'</b> '.$this->diafan->id;
		}
	}
	...
	public function edit_variable_created()
	{
		...
		echo '<b>
				...
				<input type="text" showtime="true" class="timecalendar hasDatepicker" value="'. date("d.m.Y H:m:s", $this->diafan->value).'" name="created" id="filed_created">
		</div>';
	}
		...
		if ($this->diafan->values("delivery_id"))
		{
		    ...
		    echo '<li class="item">
				<div class="item__in">
					...
					<div class="name">'.$delivery_name.'</div>
...
<div class="num"><input name="discount_summ" value="'
		...
		echo '<li class="item">
				<div class="item__in">
					...
					<div class="num"><b>'
				.(! $this->diafan->is_new ? $this->format_summ($this->diafan->values("summ")) : '')
				.'</b></div>
					<div class="num no_important ipad"></div>
	...
	public function save_variable_goods()
	{
		...
		{
			foreach($_POST["new_goods"] as $i => $price_id)
			{
				...
				$order_goods_id = DB::query("INSERT INTO {shop_order_goods} (order_id, good_id, count_goods, price, discount_id) VALUES (%d, %d, %d, %f, %d)", $this->diafan->id, $price["good_id"], $_POST["new_count_goods"][$i], $_POST["new_price_goods"][$i], $price["discount_id"]);
				$summ += $_POST["new_count_goods"][$i] * $_POST["new_price_goods"][$i];
				if($params)
				{
				...
				if(! empty($additional_costs[$price["good_id"]]))
				{
					foreach($additional_costs[$price["good_id"]] as $a)
					{
						if(! empty($_POST["additional_cost_id_good_".$row["id"].'_'.$a["id"]]))
						{
							$_POST["summ_additional_cost_price_".$price_id.'_'.$a["id"]] = $_POST["summ_additional_cost_price_".$price_id.'_'.$a["id"]] * $_POST["new_count_goods"][$i];
	...
	public function save_variable_payment_id()
	{
		...
		{
			DB::query("INSERT INTO {payment_history} (payment_id, module_name, element_id, created, `summ`) VALUES (%d, 'cart', %d, %d, %f)", $_POST["payment_id"], $this->diafan->id, time(), $this->diafan->values("summ"));
		}
	}
