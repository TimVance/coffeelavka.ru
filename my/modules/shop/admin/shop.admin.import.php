...
class Shop_admin_import extends Diafan
{
	...
	private function set_rewrite()
	{
		...
		{
			$this->diafan->_route->save($value, $this->field_value("name"), $this->id, 'shop', $this->import["element_type"], $this->import["site_id"], (! empty($this->cache["current_cat"]) ? $this->cache["current_cat"] : 0));
		}
	}
	...
	private function set_price_count()
	{
		...
		else
		{
			foreach ($count_value as $row)
			{
				...
				{
					DB::query("UPDATE {shop_price} SET count_goods=%d WHERE price_id=%d", $row["count"], $price["price_id"]);
				}
				else
				{
