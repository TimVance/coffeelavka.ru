				...
				else
				{
					...
					foreach ($param["values"] as $value)
					{
						if(! empty($result["row"]["param_multiple"][$param["id"]][$value["id"]]))
						{
							if(! $select)
							{
								$select = '<div style="display: none" class="option-list-param">'.$param["name"].' <select name="param'.$param["id"].'" class="shop-dropdown inpselect'.($result["row"]["param_multiple"][$param["id"]][$value["id"]] == 'depend' ? ' depend_param js_shop_depend_param' : '').'">';
							}
...
echo '<div class="row js_shop_buy shop_buy to-cart">';
	...
	{
		echo '<div class="col-md-12"><input type="number" autofocus min="1" value="1" name="count" class="number form-control" pattern="[0-9]+([\.|,][0-9]+)?" step="any"></div>';
	}
