...
if ($result["row"]["price_arr"])
{
	...
	echo '<div class="addict-field row">';
		echo '<div class="js_shop_form_param shop_form_param col-md-3">';
				...
				else
				{
					...
					foreach ($param["values"] as $value)
					{
						if(! empty($result["row"]["param_multiple"][$param["id"]][$value["id"]]))
						{
							if(! $select)
							{
								$select = ' '.$param["name"].' <select name="param'.$param["id"].'" class="shop-dropdown inpselect form-control'.($result["row"]["param_multiple"][$param["id"]][$value["id"]] == 'depend' ? ' depend_param js_shop_depend_param' : '').'">';
							}
...
echo '<div class="form-inline js_shop_buy shop_buy to-cart">';
	if (empty($result["row"]['is_file']) && empty($hide_submit))
	{
		echo '<input type="number" autofocus min="1" value="1" name="count" class="number form-control input-lg" pattern="[0-9]+([\.|,][0-9]+)?" step="any">';
	}
	if(empty($hide_submit))
	{
		echo '<input type="button" class="btn btn-warning btn-lg btn-buys" value="'.$this->diafan->_(' в корзину', false).'" action="buy">';
	}


if(empty($hide_submit) && ! empty($result["result"]["one_click"]))
	{
		echo '<button type="button" class="btn btn-default btn-lg" action="one_click" role="button" data-toggle="modal" data-target="#myModal"><i class="fa fa-hand-pointer-o"></i> купить в 1 клик</button>';
	}
