...
class Shop_model extends Model
{
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act] FROM {shop_category}"
	...
	private function list_brand_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, [title_meta], [act], theme, view FROM {shop_brand}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
		.(! $this->is_admin() ? " AND [act]='1'" : '')
	...
	protected function meta_brand($row)
	{
		...
		elseif($this->diafan->_route->page > 1 || $this->diafan->_route->dpage > 1)
		{
			$this->result["canonical"] = $this->diafan->_route->current_link(array("page", "dpage"));
		}
	...
	public function id()
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, $this->diafan->_site->module))
		{
			...
			$additional_cost_rels = DB::query_fetch_all("SELECT a.id, a.[name], a.percent, a.price, r.element_id, r.summ FROM {shop_additional_cost} AS a INNER JOIN {shop_additional_cost_rel} AS r ON r.additional_cost_id=a.id WHERE r.element_id=%d AND a.trash='0'", $row["id"]);
	...
	private function id_query($time)
	{
		...
		." no_buy, article, [measure_unit], access, theme, view, [act]".$fields." FROM {shop}"
		." WHERE id=%d AND trash='0' AND site_id=%d"
		.(! $this->is_admin() ? " AND [act]='1'" : "")
		...
		if ($sort == "rand" || $sort == "sale" || ! $this->result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			if(!empty($this->result["rows"]))
			{
				...
				elseif ($one_brand_id)
				{
					...
					$this->result["name"] = $brand["name"];
					$this->result["link_all"] = $this->diafan->_route->link($cat["site_id"], $brand["id"], 'shop', 'brand');
	 ...
	 * @return array
	 */
	public function show_brand($count, $site_ids, $cat_ids, $images, $images_variation)
	{
		$current_cat = false;
		if($cat_ids === "current")
		{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			$this->result["rows"] = DB::query_fetch_all("SELECT e.id, e.[name], e.[text], e.site_id
		...
		if (! $result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			if ($this->diafan->configmodules("search_brand", "shop", $site_ids[0]))
			{
				$result["brands"] = DB::query_fetch_all("SELECT b.id, b.[name], b.site_id FROM {shop_brand} AS b"
					...
					." AND (SELECT s.id FROM {shop} AS s WHERE s.brand_id=b.id AND s.[act]='1' AND s.trash='0' ".($cat_ids ? " AND s.cat_id IN (%s)" : '')." LIMIT 1)"
