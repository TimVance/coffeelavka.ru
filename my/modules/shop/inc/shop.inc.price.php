...
class Shop_inc_price extends Diafan
{
	...
	public function get_base($good_id)
	{
		...
		{
			$all_rows = DB::query_fetch_key_array("SELECT id, price_id, price, old_price, currency_id, count_goods, good_id FROM {shop_price} WHERE good_id IN  (%s) AND trash='0' AND (currency_id>0 OR price_id=id) ORDER BY currency_id DESC, price ASC, id ASC", implode(",", array_keys($this->cache["prepare_base"])), "good_id");
		...
		if($currency_id || $good_id)
		{
			...
			foreach ($currency as $c)
			{
				...
				foreach ($rows as $row)
				{
					...
					$price_id = DB::query("INSERT INTO {shop_price} (good_id, price, count_goods) VALUES (%d, %f, %d)", $row["good_id"], $new_price, $row["count_goods"]);
			...
			foreach ($rows as $row)
			{
				...
				foreach ($discounts as $d)
				{
					...
					if($in_discount)
					{
						...
						if($price != $row["price"])
						{
							DB::query("INSERT INTO {shop_price} (good_id, price, old_price, count_goods, price_id, date_start, date_finish, discount, discount_id, person, role_id) VALUES (%d, %f, %f, %d, %d, %d, %d, %f, %d, '%d', %d)", $row["good_id"], $price, $row["price"], $row["count_goods"], $row["id"], $d["date_start"], $d["date_finish"], $d["discount"], $d["id"], $d["person"], $d["role_id"]);
						}
					}
				}
			}
	...
	public function insert($good_id, $price, $old_price, $count, $params = array(), $currency_id = 0, $import_id = '', $image_id = 0)
	{
		if($import_id)
		{
			...
			if($row_i)
			{
				DB::query("UPDATE {shop_price} SET price=%f, old_price=%f, currency_id=%d, count_goods=%f WHERE id=%d", $price, $old_price, $currency_id, $count, $row_i["price_id"]);
	...
	public function send_mail_waitlist($good_id, $params, $row = array())
	{
		...
		foreach($fields as $field)
		{
			if(! isset($row[$field]))
			{
				if(! isset($old_row))
				{
					$old_row = DB::query_fetch_array("SELECT * FROM {shop} WHERE good_id=%d", $good_id);
				}
