...
//шапка таблицы
echo '<table class="table table-bordered wishlist">
	<tr>
...
if (! empty($result["rows"]))
{
	foreach ($result["rows"] as $row)
	{
		...
		echo '</td>
			...
			<td class="wishlist_remove"><span class="js_wishlist_remove" confirm="'.$this->diafan->_('Вы действительно хотите удалить товар из списка отложенных товаров?', false).'"><input type="checkbox" id="del'.$row["id"].'" name="del'.$row["id"].'" value="1"></span></td>
			<td class="js_wishlist_buy wishlist_buy">';
		if($row["buy"] &&  $result["access_buy"])
		{
			echo '<input type="button" value="'.$this->diafan->_('Купить', false).'" good_id="'.$row["id"].'" class="btn btn-danger btn-danger_mod btn-sm">';
		}
		echo '</td>
		</tr>';
	}
