...
class Install extends Diafan
{
	...
	public function start($demo)
	{
		...
		if($demo && ! is_dir(ABSOLUTE_PATH.'userfiles/demo'))
		{
			...
			{
				throw new Exception('На сервере не установлено расширение для распоковки ZIP-архивов. Распакуйте содержимое архива userfiles/demo.zip в папку userfiles/demo.');
			}
			$zip = new ZipArchive;
	...
	private function sql_rows($table, $rows)
	{
		...
		foreach($rows as $row)
		{
			if(! empty($row["module_name"]) &&  ! in_array($row["module_name"], $this->install_modules))
				continue;
	...
	private function sql_set_images($row, $element_type, $table)
	{
		...
		foreach($row["images"] as $r)
		{
			...
			{
				$path = ABSOLUTE_PATH.'userfiles/demo/original/'.$r;
			}
			$GLOBALS["image_id"] = 0;
	...
	private function sql_set_attachments($row)
	{
		...
		foreach($row["attachments"] as $r)
		{
			...
			elseif(! empty($r["is_image"]))
			{
				...
				{
					$path = 'userfiles/demo/attachments/'.$r["name"];
				}
	...
	private function sql_set_menu($row, $element_type)
	{
		...
		foreach($row["menu"] as $menu)
		{
			...
			if($image)
			{
				...
				{
					$path = ABSOLUTE_PATH.'userfiles/demo/original/'.$image;
				}
